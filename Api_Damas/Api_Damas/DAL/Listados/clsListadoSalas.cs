...
namespace Api_Damas.DAL.Listados
{
    public class clsListadoSalas
    {
            ...
            try
            {
                ...
                if (miLector.HasRows)
                {
                    while (miLector.Read())
                    {
                        ...
                        miSala.espacio = miLector.GetInt32(7);
        ...
        /// <summary>
        /// Precondiciones: No tiene.
        /// Conecta con la BBDD y saco una lista de salas con espacio
        /// Lanza los errores a la capa superior.
        /// Postcondiciones: Devuelve una lista con todas las salas con espacio
        /// </summary>
        /// <returns> List de salas. </returns>
        public static List<clsSala> ObtenerListadoSalasConEspacioDAL()
        {
            List<clsSala> listadoSalas = new List<clsSala>();
            clsMyConnection miConexion = new clsMyConnection();
            SqlConnection conexion = new SqlConnection();
            ...
            {
                conexion = miConexion.getConnection();
                miComando.CommandText = "SELECT * FROM Salas WHERE espacio < 2";
                miComando.Connection = conexion;
                miLector = miComando.ExecuteReader();
                if (miLector.HasRows)
                {
